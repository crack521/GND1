...
int getNodeNum(char* FILE_NET)
{
    ...
    int id1 = 0, id2 = 0;
    while (rd >> id1 >> id2) {
        uniqueNode.insert(id1);
        uniqueNode.insert(id2);
    }
    rd.close();
    int NodeNum = uniqueNode.count();
    return NodeNum;
}
...
int main(int argc, char* argv[])
{
    ...
    //getNodeNum
    const char* FILE_NET = path.data();  // input format of each line: id1 id2
    const char* FILE_ID = output.data();   // output the id of the removed nodes in order
    int NODE_NUM = getNodeNum(FILE_NET);// the number of nodes

    const int REMOVE_STRATEGY = atof(argv[3]); // 1: weighted method: powerIterationB(); vertex_cover_2() -- remove the node with smaller degree first
    ...
    const int TARGET_SIZE = ratio*NODE_NUM; // If the gcc size is smaller than TARGET_SIZE, the dismantling will stop. Default value can be 0.01*NODE_NUM  OR  1000
    ...
    cout << modelID << " duration: " << (double)duration/1000000 << "s" << endl;
	return 0;
}